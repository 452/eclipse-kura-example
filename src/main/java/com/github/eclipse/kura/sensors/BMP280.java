package com.github.eclipse.kura.sensors;
...
public class BMP280 {
    ...
    //
    // ----------------------------------------------------------------
    public static final int BMP280_ADDRESS = 0x76 & 0xFFFF;
    ...
    public BMP280(int addr) throws Exception {
        ...
        this.address = addr;
        I2CDeviceConfig config = new I2CDeviceConfig(I2CDeviceConfig.DEFAULT,            // I2C bus index
    ...
    public void calcSeaLevelhPa(double altitude) throws Exception {
        readTemperature();
        readPressure();
        seaLevelhPa = pressure
                * Math.pow((1 - ((0.0065 * altitude) / (temperature + 0.0065 * altitude + 273.15))), -5.257);
    }
